...
const returnString = () => Promise.resolve('string');
...
const returnBob = () => Promise.resolve({name: 'Bob'})
...
const returnList = () => Promise.resolve(['eggs', 'apples', 'milk', 'bread']);
...
const happySad = (x) => new Promise ((resolve, reject) => {
    if (x >= 1) {
        resolve("happy")
    } else {
        reject("sad")
...
const amIYourFather = (skywalker) => new Promise ((resolve, reject) => {
    if (skywalker === "Luke") {
...
const myNameIs = () => new Promise ((resolve, _) => resolve((name) => `My name is ${name}`));
