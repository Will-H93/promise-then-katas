...
const dog = async() => {
    ...
    return response.data.dogs.sort((a, b) => {
        return b.naughty - a.naughty
    })[0]
};
...
const joke = async() => {
    const jokeResponse = await fetch('jokes', "question");
    const answerResponse = await fetch('jokes')
    return {question: jokeResponse.joke, answer: answerResponse.answer}
};
